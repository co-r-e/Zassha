...
import { buildDocxSingle, buildXlsxSingle, type ImageMap } from "@/lib/exporters";
...
export default function ExportMenu({ fileId }: { fileId: string }) {
  ...
  const [busy, setBusy] = React.useState<"" | "word" | "excel">("");
  ...
  async function handle(type: "word" | "excel") {
    try {
      setBusy(type);
      ...
      if (type === "word") {
        {
          ...
          const labels = lang === "ja"
            ? {
                stepHeading: (n: number, name: string) => `### ステップ${n}: ${name}`,
                usedTool: "使用ツール",
                stepInference: "業務推察",
                timestamp: "タイムスタンプ",
                duration: "所要時間",
  ...
  async function captureImagesForContent(videoUrl: string, videoDuration: number, content: ReturnType<typeof parseMarkdownContent>): Promise<ImageMap> {
    ...
    // Capture at higher resolution for sharper Word embeds
