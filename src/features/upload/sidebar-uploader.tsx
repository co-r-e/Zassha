...
import { FileVideo, HardDrive, Maximize2, Timer, Trash2 } from "lucide-react";
      ...
      <div className="space-y-3">
        ...
        {files.length > 0 ? (
          <div className="space-y-2 max-h-40 overflow-auto pr-1">
            {files.map((sf) => {
              const state = fileStatesById[sf.id];
              return (
                  ...
                  {state && state.phase !== "idle" ? (
                    <div className="mt-1">
                      ...
                      <div className="mt-0.5 flex items-center justify-between gap-2 text-[10px]">
                        <span className={state.error ? "text-destructive truncate" : "text-muted-foreground truncate"}>
        ...
        {error ? <div className="text-[11px] text-destructive text-center">{error}</div> : null}
                ...
                <div key={sf.id} className="space-y-1">
                  ...
                  <div className="grid grid-cols-3 gap-1 text-[10px] text-muted-foreground">
                    <div className="flex items-center gap-1">
                      ...
                      <span>{videoMetaById[sf.id] ? formatDuration(videoMetaById[sf.id].duration) : "--:--"}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <Maximize2 className="h-3 w-3" />
                      <span>{videoMetaById[sf.id] ? `${videoMetaById[sf.id].width}×${videoMetaById[sf.id].height}` : "--×--"}</span>
                    </div>
                    <div className="flex items-center gap-1">
