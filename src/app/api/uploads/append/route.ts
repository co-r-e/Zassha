...
export async function POST(req: NextRequest) {
  ...
  try {
    const manifest = JSON.parse(await fs.readFile(manPath, "utf8")) as { nextIndex: number; chunkSize: number };
    if (index !== manifest.nextIndex) {
    ...
    return Response.json({ ok: true, nextIndex: manifest.nextIndex });
  } catch {
