...
<p align="center">
  <a href="https://youtu.be/gZmPmaNJeR0" target="_blank" rel="noopener noreferrer">
    <img src="https://img.youtube.com/vi/gZmPmaNJeR0/hqdefault.jpg" alt="ZASSHA デモ動画" width="640" style="border-radius:8px" />
  </a>
  <br />
  <sub>クリックして YouTube の 3 分デモを再生</sub>
</p>
...
> 最初の試行は「5 分以内・約 100MB」程度の短い動画がおすすめです。アップロードと解析が短時間で終わり、フロー確認がしやすくなります。
...
## 録画のしかた（Mac / Windows 標準機能）

- macOS（標準）
  - ショートカット: `Shift + Command + 5` → 収録範囲（画面全体/選択範囲）を選び「収録」をクリック。終了はメニューバーの停止ボタン（または `Command + Control + Esc`）。保存先は「オプション」で変更できます（既定はデスクトップ）。
  - QuickTime Player: QuickTime を開き、ファイル → 新規画面収録 → 必要に応じてマイクを選択 → 収録。終了はメニューバーから。

- Windows（標準）
  - Xbox Game Bar: `Win + G` で開き、`Win + Alt + R` で録画開始/停止。録画は既定で「ビデオ/キャプチャ」に保存されます。
  - Snipping Tool（Windows 11）: Snipping Tool を開き、録画タブに切替 → 新規 → 範囲を選択 → 開始。終了後に保存先を選びます。
